...
import type { QueryClient, QueryObserverResult, UseQueryOptions } from "@tanstack/solid-query"
import { dehydrate, hashKey, hydrate, QueryClientProvider } from "@tanstack/solid-query"
...
export function routerWithQueryClient<TRouter extends AnyRouter>(
	...
	queryClient.setDefaultOptions({
		...
		queries: {
			...
			_experimental_afterQuery: (options: UseQueryOptions, _result: QueryObserverResult) => {
				...
				;(ogClientOptions.queries as any)?._experimental_afterQuery?.(options, _result)
			},
		} as any,
	})
	...
	router.options = {
		...
		Wrap: (props: { children: JSX.Element }) => {
			const OuterWrapper = additionalOpts?.WrapProvider || ((props) => <>{props.children}</>)
			const OGWrap = ogOptions.Wrap || ((props) => <>{props.children}</>)
